    ...
    public class DACNocShiftHandOver
    {
        ...
        public DataSet GetDepartmentDetails()
        {
            try
            {
                ...
                sqlcmd.Connection = sqlcon;
                sqlcmd.CommandType = CommandType.StoredProcedure;
                sqlcmd.CommandText = "Search_Ops_DepartmentDetails";
                sqlcmd.Parameters.Add("@Date", SqlDbType.DateTime).Value = Date;
                sqlcmd.Parameters.Add("@OpsGroup", SqlDbType.NVarChar, 225).Value = OPSGroupName;
        ...
        public void createxml(int IncidentNum, string NextActionDetails,string path)
        {
            try
            {
                //Create XML Code here-- Muktesh
                ...
                XmlElement Xsource = XDoc.CreateElement("InciedentNextAction");
                XDoc.AppendChild(Xsource);

                XmlElement XTemp = XDoc.CreateElement("Tickets");

                //XTemp = XDoc.CreateElement("RecID");
                //XTemp.InnerText = RecID.ToString();
                //Xsource.AppendChild(XTemp);

                XTemp = XDoc.CreateElement("IncidentNumber");
                XTemp.InnerText = IncidentNum.ToString();
                Xsource.AppendChild(XTemp);

                XTemp = XDoc.CreateElement("Notes");
                XTemp.InnerText = NextActionDetails.ToString();
                Xsource.AppendChild(XTemp);

                
               
                string date = string.Format("{0:ddMMyyyyhhmmsstt}.xml", DateTime.Now);

                //string paths = ("\\\\FR-ITSMAPP\\NocPortalJournal\\" + IncidentNum.ToString() + "_" + date);

                string paths = (path + IncidentNum.ToString() + "_" + date); 
            ...
            ////XDoc.Save(Server.MapPath(@"\\dev-gmee\opportunity" + date +".xml"));

            //End XML Code creation

            //this.Controls.Add(txtMessageError);
            //txtMessageError.Text = "The XML document is created....";
